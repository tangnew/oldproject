(function() {
  ...
  DateRangePicker = (function() {
    ...
    function DateRangePicker($select, callback) {
      ...
      //todo jw 执行回调self.test方法，新增callback参数 add-s
	  //todo jw 执行回调self.test方法，新增callback参数 add-e
    }
   //todo jw 执行回调self.test方法，新增callback参数 add-e
    ...
    DateRangePicker.prototype.initBindings = function() {
      ...
      return this.$dateRangePicker.find('.drp-timeline-presets li').click(function(evt) {
        ...
        console.log(self.$select.val());

        //判断所选的值是否位数字 change
        if(!isNaN(self.$select.val())){
          self.hide();
        }
    ...
    DateRangePicker.prototype.hide = function() {
      ...
      return this.$dateRangePicker.hide();
            //if picker is attached to a text input, update it this.updateElement(); change
    };
    ...
    DateRangePicker.prototype.formatDate = function(d) {
      return "" + (d.getFullYear().toString()) + "-" + ((d.getMonth() + 1)<10 ? ('0'+(d.getMonth() + 1)): (d.getMonth() + 1)) + "-" + (d.getDate()<10 ? '0'+d.getDate():d.getDate());    
      // return "" +(d.getFullYear().toString().substr(0, 4))+ "-" +(Month) + "-" + (d.getDate());
      // return "" + (d.getFullYear().toString()) + "-" + ((d.getMonth() + 1)<10 ? ('0'+(d.getMonth() + 1)): (d.getMonth() + 1)) + "-" + (d.getDate()<10 ? '0'+d.getDate():d.getDate());
    };
    ...
    DateRangePicker.prototype.executeCallBack = function(){
    	 var start = this.formatDate(this.startDate());
    	 var end = this.formatDate(this.endDate());
		 //todo jw 执行回调self.test方法 add-s
    	 if(this.callback && typeof this.callback == 'function'){
    		 this.callback(start, end);
    	 }
		 //todo jw 执行回调self.test方法 add-e
  ...
  Calendar = (function() {
    ...
    Calendar.prototype.setDay = function(day) {
      ...
      // 判断是否是结束时间的点击按钮，如果是的话，则隐藏 change
    ...
    Calendar.prototype.drawDateDisplay = function() {
      ...
      return this.$dateDisplay.text([ this.year(),(this.month()<10 ? ('0'+this.month()): this.month()), (this.day()<10 ? ('0'+this.day()): this.day())].join('-'));
    };
  ...
  $.fn.dateRangePicker = function(callback) {
    return new DateRangePicker(this, callback);
  };

//todo jw 执行回调self.test方法，必须注释掉 add-s
...
//todo jw 执行回调self.test方法，必须注释掉 add-e
